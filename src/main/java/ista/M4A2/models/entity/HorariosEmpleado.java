...
@Entity
@Table(name = "Horarios_Empleado")
public class HorariosEmpleado implements Serializable {
    
    private static final long serialVersionUID = 1L;
    
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "Id_Horario_Empleado")
    ...
    @OneToMany(mappedBy = "horarioEmpleado", fetch = FetchType.LAZY)
    @JsonIgnoreProperties("horarioEmpleado")
    private List<PersonaEmpleado> empleados;
    
    // Constructores
    public HorariosEmpleado() {
    }
    
    public HorariosEmpleado(Integer idHorarioEmpleado, LocalTime horaInicioJornada, 
                           LocalTime horaFinJornada, LocalTime horaInicioDescanso, 
